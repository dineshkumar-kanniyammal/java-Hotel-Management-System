...
<title>Sort By Current Date</title>
<style>
.main-content{
width:900px;
margin:100px auto;
...
<body>
	...
	<a href="administratorprofile.jsp" class="btn btn-primary btn-lg active" role="button">Profile</a> 
 	<table class = "table table-bordered" border="2" width=200%>
	...
	for(i = 0; i < cks.length; i++) {
		if(cks[i].getName().equals("auth_checkin")) {
			date=cks[i].getValue();
	...
	PreparedStatement ps = db.getPS("SELECT * FROM USERDETAILS WHERE CHECKINDATE=? ");
	ps.setString(1,date);
	PreparedStatement ps1 = db.getPS("select * from ROOMDETAILS where CUSTOMERID IN(select CUSTOMERID from ( select * from USERDETAILS where checkindate = ?))");
